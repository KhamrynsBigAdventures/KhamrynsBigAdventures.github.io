...
(function () {
  ...
  function addStyles() {
    ...
    style.textContent = `
      .book-card{background:linear-gradient(145deg,rgba(8,45,79,.88),rgba(13,105,112,.82))!important;border-color:rgba(255,255,255,.22)!important}
      .book-art{background:linear-gradient(145deg,rgba(8,45,79,.72),rgba(22,103,168,.46))!important}
      .book-art::before{background:rgba(73,185,107,.18)!important;filter:blur(28px)!important}
      .cover-art .book-cover-frame{background:rgba(255,255,255,.04)!important}

      /* The portal is the same aqua/teal family as the site — yellow is the glow/accent. */
      .future-adventure-moon{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;width:clamp(190px,20vw,270px);height:clamp(190px,20vw,270px);margin:0;border:3px solid #ffd447;border-radius:50%;background:linear-gradient(145deg,#159fb5 0%,#18aaa9 48%,#22b7a8 100%);color:#fff;box-shadow:0 0 0 10px rgba(255,212,71,.08),0 0 45px rgba(34,183,168,.58),0 0 95px rgba(21,159,181,.42),inset -24px -28px 40px rgba(0,38,75,.18);cursor:pointer;overflow:visible;z-index:12;transition:transform .35s ease,box-shadow .35s ease}.future-adventure-moon:hover,.future-adventure-moon:focus-visible{transform:translate(-50%,-50%) scale(1.045);box-shadow:0 0 0 14px rgba(255,212,71,.12),0 0 65px rgba(34,183,168,.72),0 0 120px rgba(21,159,181,.5),inset -24px -28px 40px rgba(0,38,75,.18);outline:3px solid #ffd447;outline-offset:5px}.future-adventure-moon strong{position:relative;z-index:4;font-family:Georgia,serif;font-size:clamp(.82rem,1.2vw,1.08rem);line-height:1.18;letter-spacing:.04em;text-align:center;color:#fff;text-shadow:0 2px 10px rgba(0,38,75,.35)}.future-adventure-moon small{position:relative;z-index:4;margin-top:10px;font-size:.55rem;font-weight:900;letter-spacing:.13em;color:#fff}.moon-star{position:absolute;z-index:5;top:18%;right:21%;color:#ffd447;font-size:1.25rem;animation:moonTwinkle 1.8s ease-in-out infinite}.moon-glow{position:absolute;inset:-18px;border-radius:50%;border:1px dashed rgba(255,255,255,.38);box-shadow:0 0 35px rgba(34,183,168,.65);animation:moonSpin 14s linear infinite}.moon-craters:before,.moon-craters:after{content:'';position:absolute;border-radius:50%;background:rgba(0,70,95,.14)}.moon-craters:before{width:34px;height:34px;left:20%;top:24%;box-shadow:65px 55px 0 10px rgba(0,70,95,.10),-20px 70px 0 4px rgba(0,70,95,.10)}.moon-craters:after{width:15px;height:15px;right:20%;bottom:23%;box-shadow:-55px 25px 0 5px rgba(0,70,95,.09)}
