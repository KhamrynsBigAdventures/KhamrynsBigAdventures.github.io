...
(function () {
  ...
  function addStyles() {
    ...
    style.textContent = `
      /* Keep the book library aqua/teal and let yellow remain an accent, not a backdrop. */
      ...
      .future-adventure-moon{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;width:clamp(190px,20vw,270px);height:clamp(190px,20vw,270px);margin:0;border:3px solid #ffd447;border-radius:50%;background:radial-gradient(circle at 34% 25%,#20AAAC 0,#068C9D 38%,#057F95 68%,#064682 100%);color:#FFFFFF;box-shadow:0 0 0 10px rgba(255,212,71,.10),0 0 90px rgba(32,170,172,.75),inset -24px -28px 40px rgba(0,38,75,.28);cursor:pointer;overflow:visible;z-index:12;transition:transform .35s ease,box-shadow .35s ease}.future-adventure-moon:hover,.future-adventure-moon:focus-visible{transform:translate(-50%,-50%) scale(1.045);box-shadow:0 0 0 14px rgba(255,212,71,.13),0 0 110px rgba(32,170,172,.9),inset -24px -28px 40px rgba(0,38,75,.28);outline:3px solid #ffd447;outline-offset:5px}.future-adventure-moon strong{position:relative;z-index:4;font-family:Georgia,serif;font-size:clamp(.82rem,1.2vw,1.08rem);line-height:1.18;letter-spacing:.04em;text-align:center;text-shadow:0 1px rgba(0,0,0,.18)}.future-adventure-moon small{position:relative;z-index:4;margin-top:10px;font-size:.55rem;font-weight:900;letter-spacing:.13em;color:#FFFFFF}.moon-star{position:absolute;z-index:5;top:18%;right:21%;color:#fff4a8;font-size:1.25rem;animation:moonTwinkle 1.8s ease-in-out infinite}.moon-glow{position:absolute;inset:-18px;border-radius:50%;border:1px dashed rgba(255,216,74,.45);animation:moonSpin 14s linear infinite}.moon-craters:before,.moon-craters:after{content:'';position:absolute;border-radius:50%;background:rgba(0,70,95,.16)}.moon-craters:before{width:34px;height:34px;left:20%;top:24%;box-shadow:65px 55px 0 10px rgba(0,70,95,.12),-20px 70px 0 4px rgba(0,70,95,.12)}.moon-craters:after{width:15px;height:15px;right:20%;bottom:23%;box-shadow:-55px 25px 0 5px rgba(0,70,95,.1)}
